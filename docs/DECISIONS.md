...
## 134. DOCUMENTS, RUN 5 — 25 September 2026

Documents, Run 5 (25 Sep 2026). A report is something a person acts on, so the drawer's gaps make checklists and drafts and the facts a scan proposes finally have a door. A checklist from one gap or from all of them goes through the existing checklist engine with origin document, and its progress is read live off the items rather than stored. A draft is written into document_gaps and never into the file, it names the blanks only the customer can fill, and a second draft says it replaced the first. Nothing had ever consumed a fact_proposal: the To confirm queue is the first accept path, and a key naming one of the 95 switches goes through /api/switches/answer rather than a second way into company_switches, while everything else lands in company_facts, one row per company and key. Research-from-a-gap reuses the workspace's own pendingDoc, and an attachment nobody can see is one they cannot remove — the card for it was the fix. Three things were found by testing rather than by reading: JSON.stringify double-encoded a jsonb value, the attached document was invisible, and the report counted substep rows in a checklist's progress. And the run's own measurement stands against it: re-scanning one unchanged document renamed all five of its findings, so the title matcher that carries a checklist forward matched nothing, which is why Run 6 asks the model to name the gap it is replacing.
