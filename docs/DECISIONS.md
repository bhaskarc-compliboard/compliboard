...
Documents, Run 5 (25 Sep 2026). What a person does with a report: a checklist from one gap or from all open gaps through the existing checklist engine, with the checklist linked to its gap and its document; a draft of a missing section on the prose tier, stored on the gap with its cost row, never written into the file, blanks named for the person; research from a gap, opening the workspace with the question filled and the document attached through the attach path that already exists; and one confirmation queue, fact_proposals from both sources, with a To confirm page and sidebar count. A confirmed document fact whose key names a switch takes the existing declared-fact path; any other key is written to company_facts, one row per company per key, and shown to the next scan as what the company has confirmed. The significant date is computed when read, in the view, from kind, deadlines and the document's own date, and the stored columns are gone, because a derived value stored goes stale the first time you learn something.
