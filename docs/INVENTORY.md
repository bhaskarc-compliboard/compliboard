...
> ### ⛔ CHECKED AGAINST REALITY ON 15 SEPTEMBER 2026, AND IT IS WRONG IN SPECIFIC WAYS.
>
> **Not regenerated, and the reasons are stated so this is a judgement rather than a shrug.** What
> is wrong is the STATE of items, not the counts — most counts survive because the library and the
> switch vocabulary have not moved.
>
> **What is STILL TRUE:** 95 switches · 33 agencies · 12 switches no condition references · 42 of
> 95 with no determination path · the phase structure · every "waiting on" relationship.
>
> **WHAT IS NOW FALSE — checked, not assumed:**
>
> | Row | Says | Reality on 15 Sep |
> |---|---|---|
> | **7.2a** The routes | `⬜` *"nothing. Everything under it is built and tested"* | **BUILT.** Both routes exist and have been driven over HTTP (§91) |
> | **4.3** Determination chain | `⬜` | **BUILT** 13 Sep — the obligation writer, `/api/obligations` |
> | **7.3** obligation_evidence | `⬜` *"0 rows today"* | schema and view **built** (020/021); still 0 rows and **no writer** |
> | **Key rotation** | *"SEVEN credentials"* | **EIGHT** — `TURN_SIGNING_SECRET` created 15 Sep (§89) |
> | *"167 tests"*, twice | | **288** |
> | **6.4** | three sub-items | **four** — 6.4c, the library revision, is now the largest item in the phase |
> | M1 rows | M1 unstarted | **M1.0, M1.1, M1.2, M1.2b, M1.2c done**; `topics` on both environments |
>
> **Eleven migrations (018–028) landed after this was written and none is in it.**
>
> **Why it is kept rather than regenerated:** a 131-row inventory regenerated by hand is a second
> source of truth that will drift again within the week — which is what this block is evidence of.
> **`STATUS.md` is the current state** (one line per module with the date it was last actually
> checked) and **`TODO.md` is the current plan.** This file's remaining value is its *structure* —
> the phase-by-phase relationships and what each item waits on — which has not changed.
>
> **If it is ever regenerated it should be generated, not written** — from the database and the
> repo, the way `npm run check` reports its own numbers.

> ### 📅 A DATED SNAPSHOT, NOT A CURRENT STATE — taken **12 September 2026**.
>
