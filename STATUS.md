...
| **Obligation writer** (`lib/obligationWriter.ts`, `/api/obligations`) | 🟡 degraded | 13 Sep | **Phase 4.3, the first thing ever to write the `obligations` table — and until 023 it had never run through its own route.** The 1.95 s / 221 obligations / idempotent-on-rerun measurements were taken with a script holding the service-role key, which set `obligations_computed_at` so every later request skipped the write. The route connects as `authenticated`, which held EXECUTE on neither function it calls, so **every uncomputed company got a 500** — found by `TESTING.md` Case E on its first run (`DECISIONS.md` §63). Fixed on staging by 023; **the route path is still unexercised end to end**, which is Case E and is the owner's to run. **Not on production**, where all 10 companies remain NULL and would still 500. |
