...
**Supersedes:** version 9 (12 Sep). **Phase 7.2's schema is on production** — both environments
...
> direction, the two outputs byte-identical (sha256 `6ecf7467ec5ee534…`).** Verified 12 Sep after 013 and
...
| **Migration chain** | ✅ working | 12 Sep | **000→017 on both environments.** `npm run db:reset` rebuilds staging from empty and replays the chain; running it the first time found two migrations that had never been able to build from nothing. **012 was refused on its first attempt** — a CHECK constraint may not contain a subquery — and rolled back whole, leaving staging on 011 with nothing half-applied. That is the transaction-per-file behaviour working. |
