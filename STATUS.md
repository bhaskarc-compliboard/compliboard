...
**Supersedes:** version 15 (13 Sep). End-of-session sweep; every number re-read from the databases
...
| **Migration chain** | 🟡 degraded | 13 Sep | **The two environments differ by one migration.** Staging is on **000–024** (`applied 25, latest 024`), production on **000–023** (`applied 24, latest 023`); 25 files on disk. **024 recounts `industry_coverage.row_count` and is not on production**, where OR-OSHA still reads 53 against a live 61 (`AUDIT-CHECKS.md` check 4). Object census **before** 024: **852 objects each side, sha256 identical, 0 differences across 10 categories** — 26 tables, 1 view, 394 columns, 94 enum labels, 6 functions, 80 indexes, 71 policies, 121 constraints, 13 triggers, **0 `anon` grants either side**. `npm run preflight` prints both inputs and derives the pending list in front of the reader. |
...
| **Switch determination + ask path** (7.2 / 7.2a) | ⬜ **routes not built** | 13 Sep | **`/api/switches/ask` and `/api/switches/answer` DO NOT EXIST.** Derived by grep, not recalled: `app/api/` holds 18 routes and none is `switches`. **Nothing in `app/` has ever written a `company_switches` row** — Test Alpha's 16 facts came from `scripts/seed-multisite-fixture.js:141`. Four modules have no production caller: `switchAsk` (6 exports), `switchDetermination` (4), `sdsExtraction` (4), `basis` (3, only via switchDetermination). ~60 tests, zero requests. **This is M1's floor** — M1's central interaction is fact capture and there is no endpoint for it. |
