...
**Supersedes:** version 8 (12 Sep). **Phase 4.1 is on production** — both environments on
...
**Environment:** **staging and production both on 000–016**
...
> values, table ACLs and RLS flags — **799 objects each, zero differences, in either
> direction, the two outputs byte-identical (sha256 `2407ffbcebcddc66…`).** Verified 12 Sep after 013 and
...
| **Migration chain** | ✅ working | 12 Sep | **000→016 on both environments.** `npm run db:reset` rebuilds staging from empty and replays the chain; running it the first time found two migrations that had never been able to build from nothing. **012 was refused on its first attempt** — a CHECK constraint may not contain a subquery — and rolled back whole, leaving staging on 011 with nothing half-applied. That is the transaction-per-file behaviour working. |
...
| **Switch library** (`switches`) | ✅ working | 12 Sep | **95 switches in both environments, 40 dependency edges, 0 cycles, graph walked from each database and compared** — 95 nodes reached, 55 roots, 40 children, max depth 1, depth maps identical. 73 site-scoped, 22 company; 6 numeric carrying their thresholds. Derived from 188 requirement trigger strings rather than from the design document — `DECISIONS.md` §36. **12 of the 95 are referenced by no requirement condition**; 4 of those are context rather than triggers, 8 are the 6.3b backlog. |
| **Requirement conditions** (`applies_expression`) | 🟡 degraded | 12 Sep | **199 of 200 live requirements carry a machine-evaluable condition, in both environments, byte-identical.** 83 distinct switches referenced, **0 dangling**, 0 retired rows carrying one. The 200th is `Employee handbook` — cited as "Best practice (not statute)", so there is no condition under which it applies; deliberately NULL. **Degraded because confidence is not in the database.** 175 conditions are high-confidence, 18 medium and 6 low, with the reasoning for each written in `supabase/seed-data/applies-expressions.json` — and the loader writes only `applies_expression`, so nothing downstream can tell a quoted threshold from a proxy. `CLAUDE.md` §6 says a verified row and a generated row must not look the same; here they do. |
| **Resolution engine** (`lib/resolve.ts`, `lib/jurisdiction.ts`) | ✅ working | 12 Sep | **Layer 3, and it contains no AI.** Jurisdiction + switches + library → obligations, deterministically. Six-case match key as a pure predicate; company-level expressions evaluated once per site and combined with a three-valued existential; `unknown` never resolves to `does_not_apply` anywhere in the chain. **119 tests, and all nine safety properties proved by breaking them** — `npm run mutation` applies nine plausible wrong implementations and all nine are caught. **Not wired to a route.** It runs from `scripts/resolve-dryrun.js` against staging only. |
| **`close_and_replace_obligations`** | ✅ working | 12 Sep | **In both environments, migration 016.** Closes changed or departed obligations by setting `applicable_to` and inserts the new ones, in one transaction — **no DELETE**, because obligations are history (`DECISIONS.md` §47). `SECURITY INVOKER`, `search_path=''`, ownership raised in SQL, EXECUTE held by `service_role` and `postgres` only (verified with `aclexplode`, both sides). Exercised live on **staging only**: idempotent on a repeat run, a status change closes one and opens one, a departed requirement is closed not deleted, a cross-tenant `entity_id` is refused before any write, and **a fault injected mid-INSERT rolls the close back with it**. Deliberately never called against production. |
