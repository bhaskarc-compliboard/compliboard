...
**Supersedes:** version 7 (12 Sep). Documentation sweep: every count below re-read from the
...
> values, table ACLs and RLS flags — **798 objects each, zero differences, in either
> direction, the two outputs byte-identical (same sha256).** Verified 12 Sep after 013 and
> 014 and the expression load. `anon` holds nothing on either side.
>
