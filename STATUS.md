...
**Supersedes:** version 6 (12 Sep). **Phase 6.3 is on production.** Both environments carry
migrations 013 and 014, 205 requirement rows with 199 machine-evaluable conditions, and 95
switches — verified by a census that now lives in `supabase/census.sql` rather than being
retyped each run. Version 6 recorded that **Phase 2 is closed.** Records the measured per-stage
...
**Environment:** **staging and production both on 000–014**
...
| **Migration chain** | ✅ working | 12 Sep | **000→014 on both environments.** `npm run db:reset` rebuilds staging from empty and replays the chain; running it the first time found two migrations that had never been able to build from nothing. **012 was refused on its first attempt** — a CHECK constraint may not contain a subquery — and rolled back whole, leaving staging on 011 with nothing half-applied. That is the transaction-per-file behaviour working. |
| **Requirement library** | 🟡 degraded | 12 Sep | **205 rows in both environments — 200 live, 5 retired parents, 17 split children with resolved lineage** (migration 013 split Electronic OSHA injury-data submission into 3, Process Safety Management into 5, and Permit-required confined spaces into 3), every row categorised and 187 assigned a regulator. **Degraded on one axis and it is the important one: 0 of 194 carry a `citation_url`, a `citation_quote` or a `source_checked_at`, and 0 are at `status = 'verified'`.** Every row names a rule and not one links to it. `AUDIT-CHECKS.md` check 1. |
