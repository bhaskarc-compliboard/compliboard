...
- **THE DOCUMENT-REVIEW PATH SPENDS MONEY NOTHING COUNTS.** `lib/documentReview.ts` calls
  `askAIJson` with **no `ledger:` argument** (`grep -n ledger lib/documentReview.ts` returns
  nothing), so every document scan is absent from `ai_calls` and from every figure
  `npm run cost` prints. **The owner measured a two-page PDF scan at $1.10 on live on
  23 September** — that one call is more than the entire `convert` task's recorded spend, and
  the ledger does not know it happened. `npm run cost` names the gap every run: six of ten
  tasks have never written a row — `substeps`, `gate`, `critique`, `audit`, `document_review`,
  `other`. **The totals are a floor, not a total**, and this is the largest known piece missing
  from them. `TODO.md` M4; it is where the Documents chat starts.
