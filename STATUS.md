...
**Supersedes:** version 14 (13 Sep). Sweep after 7.2a, 7.3, M6 and 4.3, and after the manual set's
...
| **Migration chain** | ✅ working | 13 Sep | **000→023 on BOTH environments**, read from `supabase_migrations.schema_migrations`: staging and production both report `applied 24, latest 023`. **Object-for-object after 023: 0 differences across 10 categories** — 26 tables, 1 view, 394 columns, 94 enum labels, 6 functions, 80 indexes, 71 policies, 121 constraints, 13 triggers, **0 `anon` grants on either side**. `npm run db:reset` rebuilds staging from empty and replays the chain. `npm run preflight` prints both inputs in full and derives the pending list in front of the reader; **it held the one time it was tested** — an abort on a reported filename mismatch, settled by one `ls` (`DECISIONS.md` §65). |
