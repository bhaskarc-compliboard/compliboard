...
**Version:** 23 · **Updated:** 23 September 2026
**Supersedes:** version 22 (23 Sep). **RUN 3** (`DECISIONS.md` §126): `app/compliance/page.tsx` is
rebuilt from the prototype — tabs, docking composer, drawers, rendered markdown, citation cards,
readable source titles, a working state driven by real stream events, and uploads finishing Run
2's Task 5 from the client side. **The research/checklist section is code-complete and NOT yet
signed off**: `check:live` proves the routes, and the ten manual actions in `TESTING.md` are the
owner's pass before it ships. `GateAskCard` is archived and the gate-on path is unhandled until
R1.5. Version 22: **RUN 2** (`DECISIONS.md` §125): conversations persist, the
...
| **The workspace page** (`app/compliance`) | 🟡 **built, awaiting the manual pass** | **23 Sep** | **§126 — rebuilt from `prototypes/compliance-workspace.html`.** Three tabs, a composer that docks after the first question, drawers over the conversation, rows with hairline separators. **Markdown renders** (react-markdown + remark-gfm — Run 1 showed raw pipes); `[n]` opens a card with title, domain and link; junk source titles fall back to a URL-derived name (`lib/sourceTitle.ts`, 19 tests). The working state comes from **real stream events**, not a timer. `Delete` is always visible below 820px — touch has no hover. **🟡 because the routes are proved by `check:live` and the SCREEN is not**: `TESTING.md`'s ten manual actions are the owner's pass, and nothing here claims they have been run. **Known limitation: `outcome: 'ask'` is not handled**, so the gate cannot be switched on before R1.5 (`components/archive/GateAskCard.tsx`). |
...
| **Usage counters** | ✅ working | **23 Sep** | **§125 — events, not inventory.** Never decremented, never derived from row counts: transcripts are cleared after 7 days and checklists can be deleted, and a `count(*)` would tell a customer who asked forty questions that they asked six. Incremented by an atomic database function (PostgREST cannot express `col = col + 1`), whose EXECUTE is revoked from PUBLIC in the same migration. Proved: answer +1, stop unchanged, delete a checklist unchanged. |
| **Nightly jobs** (`/api/jobs/*`) | ✅ working | **23 Sep** | **§125, §116's gates 1 and 2.** Vercel Cron, two protected routes, **no worker**. They refuse when `CRON_SECRET` is UNSET rather than when it mismatches, and answer 404 so the route cannot be confirmed to exist. Summariser writes the summary, proposes facts into `fact_proposals` and **never writes `company_switches`** (§108); it never overwrites a user's summary without newer turns. Deleter clears transcripts, keeps topic and summary, and carries a **30-day backstop** so a summariser outage cannot make transcripts permanent. One `job_runs` row per run, counts by topic id. |
| **Conversation → checklist** | ✅ working | **23 Sep** | **§125 — scope and provenance.** `discussed` may cite only sources the conversation already cited, **enforced in code after the call** and items citing anything else are dropped and counted. `complete` adds the rest as `origin='added'`. Title names the scope; `from_topic_id` is stored, ON DELETE SET NULL so clearing a transcript never deletes the checklist. **§111's obligation link stays deferred** — `origin` is provenance, not authority. |
| **Research answer** (`/api/chat` research) | ✅ **open baseline** | **22 Sep** | **§123, R1.0 — one sentence of role, search the model decides on, history as plain pairs, STREAMED.** No gate, facts block, frame, scenario block or template: all six are switches (`lib/pipelineConfig.ts`) and production runs them off. Driven as `testgamma` on staging: **33.1 s / 6.2 s / 11.3 s** on the three benchmark questions, first text at **1.3–2.8 s**, 8 sources on the one that searched. The **narration rule** strips the model's "I'll search for…" opening — positional, from three recorded samples, 10 tests. Abort reaches the SDK, proved from the server log. **Answer QUALITY is unmeasured here on purpose** — it is the owner's to judge against an incognito chat (§115). |
| **Checklist answer** (`/api/chat` checklist) | ✅ **open baseline** | **22 Sep** | **§123 — the same open call, and a SHAPE rather than a fence.** The prompt is the role sentence plus the output shape and nothing else; `cost_note`, `providers` and `safety_alert` are gone from it as slots the model fills because they exist (§113), and stay optional in the type because 235 production items carry them. Driven as `testgamma`: 8 `must_do`, 7 `good_to_have`, shape intact. Gate and critic sit behind `CHECKLIST_GATE` / `CHECKLIST_CRITIC`, both off. **Still writes checklist rows with no link to an obligation — §111's hybrid is NOT built.** |
| **Micro-steps** (`substeps`) | ✅ working | **22 Sep** | **§123 — generated in the BACKGROUND after the list renders, at most 3 in flight.** Was serial and on-demand, so every first expand waited on a model call. **Never lost, never repeated**: the enqueue condition is absence from storage, so restored items are skipped forever and unfinished ones resume next open. They **inherit the parent item's sources and carry none of their own**. Stored as today, by `parent_item_index`. |
| **Critic pass — coverage** | ✅ working | 13 Sep | **Routed since 12 Sep** — `/api/chat:184` and `/api/audits:427` — and **tested since 13 Sep: 24 assertions** over `normaliseCritique` and `applyCritique`. It had none before because it could not be IMPORTED by one: `@/lib/...` is resolved by the bundler and by `tsc`, **not by Node's loader**, which runs the suite (`DECISIONS.md` §67). Three files are still in that state — `documentReview`, `determinationGate`, `documentContent` — all three on M1's path. **Latency untouched and it is M1's real risk: 84 s average, 37–151 s**, because Stage 2 does not exist to narrow 33 agencies. |
