...
**Supersedes:** version 11 (12 Sep). **The chain runs end to end for the first time.** A question is
...
| **Obligation writer** (`lib/obligationWriter.ts`, `/api/obligations`) | ✅ working | 13 Sep | **Phase 4.3, and the first thing ever to write the `obligations` table.** Lazy — computes on the first GET when `obligations_computed_at` is NULL, not at signup (`DECISIONS.md` §62). Measured on Test Alpha: **1.95 s for 221 obligations across 2 entities**, `{closed:0, opened:221, unchanged:0}`. **Idempotent, verified by running it twice**: the second run reported `{opened:0, unchanged:221}`. Migration 022 adds `obligations_computed_at`, which is what separates "never worked out" from "worked out, and nothing applies" — a row count cannot. **On production the column exists and every one of the 10 companies is NULL**, which is correct: the write fires on a GET and nobody has opened the screen. |
